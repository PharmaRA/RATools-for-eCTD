...
/// 同理 BookmarkMaxDepth / PageMode 读不到时报 null，规则对 null 一律不发 finding。
/// </summary>
public sealed record PdfInspectionResult(
    ...
    int? PageCount = null,
    // 书签树最大层数（根节点算第 1 层）；无书签为 0，读不到为 null。
    int? BookmarkMaxDepth = null,
    // catalog 的 /PageMode 值（如 UseOutlines），未声明或读不到为 null。
